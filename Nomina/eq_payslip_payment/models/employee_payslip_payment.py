    ...
    def default_get(self,fields):
        ...
        if self._context.get('payslip_ids') and self._context.get('active_model') == 'hr.payslip':
            ...
            for payslip in payslip_ids.filtered(lambda l:l.state =='done'):
                ...
                payment_lines = {'employee_id':payslip.employee_id.id,'number':payslip.number,
                                 'payslip_id':payslip.id,
                    'payslip_due_amount':(payslip.pay_amount - due_amount),'company_id':payslip.company_id.id,
                    'paid_amount':(payslip.pay_amount - due_amount),'currency_id':payslip.company_id.currency_id.id}
